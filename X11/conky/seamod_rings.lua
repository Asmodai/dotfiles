...
gauge = { 
   ...
   -- System info rings
   {
      ...
      --conky_line='${if_up eth0}${upspeedf eth0}${else}${if_up wlan0}${upspeedf wlan0}${endif}${endif}',
      conky_line='${exec ~/.conky/backlight.pl}',
      name='Brightness',
      arg='',
      max_value=100,
      x=70,
...
function battery_watch()
   ...
   batt=tonumber(conky_parse("${battery_percent BAT1}"))
   ...
   batt_status=(string.find(conky_parse("${battery_short BAT1}"), "D"))
...
function mute_watch()
   mute=tonumber(conky_parse("${exec amixer sget 'Master' | grep off | cut -c 25 | head -1}"))
   if mute==nil then
